    ...
    public Collection<GraphNode> buildFromEdges(Iterator<GraphEdge> edges) {
        ...
        for (GraphEdge graphEdge : edgeList) {
            ...
            if (graphEdge.isDescriptiveEdge()) {
                toNode.biFunction = checkNotNull(
                        resolver.resolveStringOperation(graphEdge.getOperationName().toLowerCase()),
                        String.format("operation \"%s\" cannot be resolved as a string operation",
                                graphEdge.getOperationName()));
            }
        }
